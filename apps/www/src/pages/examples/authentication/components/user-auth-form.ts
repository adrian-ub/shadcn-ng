...
@Component({
  ...
  <div [class]="computedClass()">
    <form (submit)="onSubmit($event)">
      <div class="grid gap-2">
        <div className="grid gap-1">
          <label ubLabel class="sr-only" htmlFor="email">Email</label>
          <input ubInput id="email" type="email" placeholder="name@example.com" [disabled]="isLoading()" />
        </div>

        <button ubButton [disabled]="isLoading()" type="submit">
          @if(isLoading()) {
    ...
    <button ubButton variant="outline" [disabled]="isLoading()">
      <ng-container *ngTemplateOutlet="isLoading() ? loadingIcon : gitHubIcon"></ng-container>
      GitHub
    </button>
  </div>
...
export class UserAuthFormComponent {
  ...
  computedClass = computed(() => cn('grid gap-6', this.class()))
