...
- `@shadcn-ng/button` - UI component from the shadcn-ng registry
...
```bash
npx shadcn-ng@latest add @v0/dashboard
```

or multiple resources at once:

```bash
npx shadcn-ng@latest add @acme/header @lib/auth-utils @ai/chatbot-rules
```
...
```bash
npx shadcn-ng@latest add @acme/button
```
...
- Examples of valid names: `@acme-ui`, `@my_company`
...
> **Note:** The `{name}` placeholder in the URL is automatically parsed and replaced with the resource name when you run `npx shadcn-ng@latest add @namespace/resource-name`. For example, `@acme/button` becomes `https://registry.acme.com/resources/button.json`. See [URL Pattern System](#url-pattern-system) for more details.
...
```bash
npx shadcn-ng@latest view @acme/button
```
...
```json title="registry-item.json" showLineNumbers
{
  ...
  "registryDependencies": [
    "@shadcn-ng/card", // From default registry
...
When you run `npx shadcn-ng@latest add @namespace/resource`, the CLI does the following:
...
```bash
npx shadcn-ng@latest add @acme/auth @custom/login-form
```
...
```bash
npx shadcn-ng@latest add @my-company/custom-button
```
...
```json title="extended-table.json" showLineNumbers
{
  ...
  "files": [
    {
      "path": "components/ui/table-extended.ts",
      "content": "import { Table } from '@vendor/table'\n// Add your extensions\nexport class ExtendedTable { ... }"
    }
  ]
}
```

This will install the original table from `@vendor/table` and then add your extensions to `components/ui/table-extended.ts`.
...
```json title="dashboard.json" showLineNumbers
{
  "name": "dashboard",
  "registryDependencies": [
    "@shadcn-ng/card", // 1. Resolved first
    "@vendor/chart", // 2. Resolved second
    "@custom/card" // 3. Resolved last (overrides @shadcn-ng/card)
  ]
}
```
...
1. `@shadcn-ng/card` - installs to `components/ui/card.ts`
2. `@vendor/chart` - installs to `components/ui/chart.ts`
3. `@custom/card` - overwrites `components/ui/card.ts` (if same target)
...
The shadcn-ng CLI provides several commands for working with namespaced registries:
...
npx shadcn-ng@latest add @acme/dashboard

# Install multiple resources
npx shadcn-ng@latest add @acme/button @lib/utils @ai/prompt

# Install from URL directly
npx shadcn-ng@latest add https://registry.example.com/button.json

# Install from local file
npx shadcn-ng@latest add ./local-registry/button.json
```
...
npx shadcn-ng@latest view @acme/dashboard @shadcn-ng/card

# View from URL
npx shadcn-ng@latest view https://registry.example.com/button.json
```
...
npx shadcn-ng@latest search @acme

# Search with query
npx shadcn-ng@latest search @acme --query "auth"

# Search multiple registries
npx shadcn-ng@latest search @acme @lib

# Limit results
npx shadcn-ng@latest search @acme --limit 10 --offset 20
...
```bash
npx shadcn-ng@latest add @non-existent/component
```
