...
export async function updateFiles(
  ...
  const [maybeBaseColor] = await Promise.all([
    getRegistryBaseColor(config.tailwind.baseColor),
  ])
  const baseColor = typeof maybeBaseColor === 'undefined' ? undefined : maybeBaseColor
  ...
  for (const file of files) {
    ...
    if (existingFile) {
      ...
      const [normalizedExisting, normalizedNew] = await Promise.all([
        ...
        getNormalizedFileContent(typeof content === 'string' ? content : content.getText()),
      ])
    ...
    await fs.writeFile(
      filePath,
      typeof content === 'string' ? content : content.getText(),
      'utf-8',
    )
    existingFile
...
export function resolveFilePath(
  ...
  if (file.target) {
    ...
    return path.join(config.resolvedPaths.cwd, 'src', target.replace('src/', ''))
  }
...
async function resolveImports(filePaths: string[], config: Config): Promise<string[]> {
