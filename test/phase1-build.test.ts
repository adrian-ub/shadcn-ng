...
describe('P1.T2: Build migration — tsdown config', () => {
  it('has tsdown.config.ts with tsdown import', () => {
    const configPath = resolve(projectRoot, 'tsdown.config.ts')
  ...
  it('has tsdown available in project dependencies', () => {
    const pkgPath = resolve(projectRoot, 'package.json')
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8'))

    // tsdown could be in root devDeps or in workspace catalog
    const hasTsdown = pkg.devDependencies?.tsdown !== undefined
    expect(hasTsdown).toBe(true)

    // unbuild should no longer be in root devDeps
