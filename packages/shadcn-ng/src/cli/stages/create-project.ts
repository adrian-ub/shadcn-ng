...
export async function createProject(options: Pick<InitOptions, 'cwd' | 'force' | 'components'>): Promise<{ projectPath: string }> {
    ...
    const { value: name } = await prompts({
      ...
      validate: (value: string) => {
        ...
        if (fs.existsSync(path.resolve(options.cwd, value, 'package.json'))) {
          return `A project with the name ${highlighter.info(value)} already exists.`
        }
        return true
      },
  ...
  const createSpinner = spinner(`Creating project ${highlighter.info(projectName)}...`).start()
  ...
  createSpinner.succeed(`Project ${highlighter.info(projectName)} created.`)
  return {
