...
export async function updateFiles(
  ...
  if (!hasUpdatedFiles && !filesSkipped.length) {
    filesCreatedSpinner.text = 'No files updated.'
  }
  ...
  if (filesUpdated.length) {
    logger.info(`Updated ${filesUpdated.length} ${filesUpdated.length === 1 ? 'file' : 'files'
    }:`)
  ...
  if (filesSkipped.length) {
    logger.info(`Skipped ${filesSkipped.length} ${filesUpdated.length === 1 ? 'file' : 'files'
