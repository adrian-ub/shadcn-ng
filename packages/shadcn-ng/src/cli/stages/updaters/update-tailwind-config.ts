...
export async function updateTailwindConfig(
  ...
  const tailwindSpinner = spinner(`Updating ${highlighter.info(tailwindFileRelativePath)}`).start()

  const raw = await fs.readFile(config.resolvedPaths.tailwindConfig, 'utf8')
  const output = await transformTailwindConfig(raw, tailwindConfig, config)
  await fs.writeFile(config.resolvedPaths.tailwindConfig, output, 'utf8')
  tailwindSpinner.succeed(`Updated ${highlighter.info(tailwindFileRelativePath)}`)
}

export async function _createSourceFile(input: string, config: Config | null): Promise<SourceFile> {
...
export function nestSpreadProperties(obj: ObjectLiteralExpression): void {
...
export function nestSpreadElements(arr: ArrayLiteralExpression): void {
...
export function unnestSpreadProperties(obj: ObjectLiteralExpression): void {
  const properties = obj.getProperties()
...
export function unnsetSpreadElements(arr: ArrayLiteralExpression): void {
  const elements = arr.getElements()
  for (let j = 0; j < elements.length; j++) {
    const element = elements[j]
    if (element.isKind(SyntaxKind.ObjectLiteralExpression)) {
